...
@interface BD_SMVDetailSettingMessageParamSetBaseView()<UITextFieldDelegate>
...
-(NSMutableArray *)blockDataArr{
    if (!_blockDataArr) {
        _blockDataArr = [[NSMutableArray alloc]init];
    }
    return _blockDataArr;
}
-(void)createSubView{
    ...
    NSMutableArray *blockArr1 = [[NSMutableArray alloc]init];
    NSMutableArray *blockArr2 = [[NSMutableArray alloc]init];
    for (NSInteger i = 0; i<_viewTitleArr.count; i++) {
        ...
        if (i == 0) {
            ...
            for (NSInteger n = 0;n<columns.count;n++) {
                ...
                [blockArr2 addObject:tf.text];
            }
        }
        
    }
    [self.blockDataArr addObject:blockArr1];
    [self.blockDataArr addObject:blockArr2];
}


-(void)textFieldDidEndEditing:(UITextField *)textField{

    for (NSInteger i=0; i<_subViewsArr1.count; i++) {
        if ([textField isEqual:_subViewsArr1[i]]) {
            [self.blockDataArr[0]replaceObjectAtIndex:i withObject:textField.text];
            break;
        }
    }
    for (NSInteger i=0; i<_subViewsArr2.count; i++) {
        if ([textField isEqual:_subViewsArr2[i]]) {
            [self.blockDataArr[1]replaceObjectAtIndex:i withObject:textField.text];
            break;
        }
    }
}
